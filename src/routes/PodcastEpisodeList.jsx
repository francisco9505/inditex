...
export function PodcastEpisodeList({list}) {
  ...
  return (<div className="podcast__episode-list">
    <div className="podcast__episode-list-count podcast__card">Episodes: {list.length}</div>
    <div className="podcast__card">
      <table className="podcast__table">
        ...
        <tbody>
        {list.map(({trackId, trackName, releaseDate, trackTimeMillis}, index) => (
          <tr className={index % 2 === 0 ? 'podcast__episode-row gray' : 'podcast__episode-row'} key={trackId} onClick={() => openTrack(trackId)}>
